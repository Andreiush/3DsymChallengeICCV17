...
template<typename VecA, typename VecB>
inline void
projectDirection(VecA const& P, VecB const& pt, VecB &projPt)
{
  VecB n;
  for(int i = 0; i < P.size()-1; ++i)
    n[i]=P[i];
...
int scoreSymmetryPlane(vector<Vector3f> const &points, Vector4f P, float radius,
...
double computeSymmetry(vector<float> const &verts,
                       Vector4f &P, std::vector<int> &inliers, double inlierThreshold,
                       vector<Vector4f> &planes, vector< vector<int> > &ins,
                       int Nplanes, double planeThresh, cv::flann::Index &kdtree)
{
  int N = verts.size()/3;
  vector<Vector3f> pointsInBB;
  for(int i=0; i<verts.size(); i+=3)
  {
    Vector3f pt(verts[i], verts[i+1], verts[i+2]);
  ...
  expectedRuns = min(ransacRuns(2,0.05, confidence),500);
...
float getPlane(Vector4f P, const vector<float> &verts,
               const vector<int> &inliers, vector<Vector3f> &plane)
{
  ...
  Vector3f zOr(0,0,1),z;
  projectDirection(P,zOr,z);
  if(z.norm()<0.01)
  {
    z[0]=1;z[1]=0;z[2]=0;
    projectDirection(P,zOr,z);
...
main(int argc, char * argv[])
{
  ...
  desc.add_options()
      ...
      ("modelList", boost::program_options::value<std::string>(&modelList)->default_value("testGlob.txt"), "Model file list")
      ...
      ("nPlanes", boost::program_options::value<int>(&nPlanes)->default_value(3),"number of planes per box")
  ...
  for (unsigned int i = 0; i < modelFileNames.size(); i++)
  {
    ...
    cout<<"Num points "<<verts.size()/3<<endl;
    cleanVerts(verts,faces);
    cout<<"Num clean points "<<verts.size()/3<<endl;

    aabb bbox = computeBbox(verts);

    int N = verts.size()/3;
    maxDist = (bbox.maxX-bbox.minX);
    ...
    computeSymmetry(verts,P,inliers,radius,planes,ins,
                    nPlanes, thresh,kdtree);
    ...
    for(int j=0;j<n;++j)
    {
      ...
      vector<Vector3f> P;
      float dim = getPlane(planes[j],verts,
                           inliers, P);
